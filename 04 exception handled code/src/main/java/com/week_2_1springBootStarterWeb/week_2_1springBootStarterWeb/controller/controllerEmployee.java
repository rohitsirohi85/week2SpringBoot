...
public class controllerEmployee {
    ...
    public ResponseEntity<employeeData> getEmployeeById(@PathVariable(name = "employeeId") Long id) {
        ...
        return employeeData
                ...
                .orElseThrow(() -> new NotFoundException("Employee not found with id" + id)); // if given employee id
                                                                                              // data not found it will
                                                                                              // throw
                                                                                              // NoSuchElementException
    }

    /*
     * it will handle the exception only in controller ...noy globally for globally
     * go to the adviceExceptionHandlePackage
     */

    // @ExceptionHandler(NoSuchElementException.class) // so it will handle the
    // NoSuchElementException from code
    // public ResponseEntity<String> hadlingEmployeeNotFound(NoSuchElementException
    // exception){
    ...
    public ResponseEntity<List<employeeData>> getAllEmployees() {
        // Call the service method to get all employees and return a 200 OK response
    ...
    public ResponseEntity<employeeData> createNewEmployee(@RequestBody @Valid employeeData inputEmployee) {
        // Call the service method to create a new employee and get the saved employee
        // data
    ...
    public ResponseEntity<employeeData> updateEmployeeById(@PathVariable  Long employeeId,
            @RequestBody @Valid employeeData employeeData) {
    ...
    public ResponseEntity<Boolean> deleteEmployeeById(@PathVariable Long employeeId) {
        ...
        // If the deletion was successful (gotDeleted is true), return a 200 OK response
        // with true
        if (gotDeleted) {
            return ResponseEntity.ok(true);
        } else {
            // If the deletion was not successful (gotDeleted is false, likely because the
    ...
    public ResponseEntity<employeeData> updatePartiallyEmployeeById(@PathVariable Long employeeId,
            @RequestBody Map<String, Object> update) {

        // @PatchMapping: Specifies that this method will handle PATCH requests to
        // "/{employeeId}".
        // @PathVariable: Extracts the employeeId from the URL path.
        // @RequestBody: Indicates that the method parameter update will be populated
        ...
        // Check if the returned employeeData is null (i.e., the employee with the given
        // ID does not exist)
        if (employeeData == null) {
            // Return a 404 Not Found response if the employee does not exist
            return ResponseEntity.notFound().build();
        } else {
            // Return a 200 OK response with the updated employee data if the update was
            // successful
